...
class ReactionTimer(object):
    ...
    def __init__(self, master):
        ...
        colormenu.add_command(
            label='blue', command=lambda: self.set_color('blue'))
        colormenu.add_command(
            label='red', command=lambda: self.set_color('red'))
        colormenu.add_command(
            label='green', command=lambda: self.set_color('green'))

        guidemenu = Menu(menu, tearoff=0)
        menu.add_cascade(label='info', menu=guidemenu)
        guidemenu.add_command(
            label='guide', command=lambda: self.show_text('guide'))
        guidemenu.add_command(
            label='about', command=lambda: self.show_text('about'))

        # status bar
        self.status = Label(
            master, text='click screen ONCE to start', bd=1, relief=SUNKEN, anchor=W)
    ...
    def on_click(self, e):
        if self.state == 0:
            ...
            tkinter.messagebox.showinfo(
                message='Don\'t cheat!', title='warning')
    ...
    def start(self):
        ...
        # canvas
        if self.state == 1:
            self.r = self.canvas.create_rectangle(
                50, 25, 150, 75, fill=self.color, outline=self.color)
    ...
    def end(self):
        ...
        # show info
        self.status.config(
            text='Reaction time: {:.3f}s'.format(self.reaction_time))
        self.state = 0
